...
addMissionEventHandler ["ExtensionCallback",{
	...
	if (_name == EXT_var_extensionName) then {
		...
		if (_method == "ASYNC_RESPONSE") then {
			...
			// IS data that we need to process (call in from extension)
			// call compile / extension requesting data / etc ...
			switch (_method) do {
				case "KickPlayer": {
					diag_log "KickPlayer";
					// Kick player x
				...
				case "RequestPlayers": {
					diag_log "RequestPlayers";
					// Send player list back to extension
				};
